...
def series_master(ev):
    """The repeating CalDAV event an occurrence belongs to — itself, or the series a changed
    occurrence was taken from; None for an event that does not repeat (and for Google's)."""
    if is_google(ev): return None
    if getattr(ev, "master", None) is not None: return ev.master
    return ev if getattr(ev, "rrule", "") else None


def as_datetime(d):
    return d if isinstance(d, datetime) else datetime(d.year, d.month, d.day, tzinfo=LOCAL)


def kept_lines(ev, exdates=True):
    names = ("CREATED", "SEQUENCE", "CLASS", "STATUS", "TRANSP", "CATEGORIES") + (("EXDATE",) if exdates else ())
    return [l for l in ce.top_level(ev.lines) if l.split(":", 1)[0].split(";", 1)[0].upper() in names]
...
class Main(QtWidgets.QMainWindow):
        ...
        def fetch():
            ...
            for name, url in cals:
                ...
                for ev in events:
                    ...
                    # occurrences of a series changed one by one (on the phone, say): each is an
                    # event of its own in the same file, shown in place of the one it replaces
                    for ov in getattr(ev, "overrides", []):
                        if ov.cancelled: continue
                        ov.cal_url = url; ov.writable = ev.writable
                        for s, e in ov.occurrences(ws, we):
                            out.append(Occ(ov, name, s, e))
    ...
    def move_event(self, o, days, minutes):
        ...
        if series_master(ev) is not None:
            delta = timedelta(days=days, minutes=0 if ev.all_day else minutes)
            chosen = self.ask_scope("move", o, {
                "this": lambda: self._series_write(o, "this", delta=delta),
                "following": lambda: self._series_write(o, "following", delta=delta),
                "series": lambda: self._do_move(o, days, minutes)})
            if not chosen:
    ...
    def _do_move(self, o, days, minutes, series=False):
        ...
        ev = series_master(ev) or ev     # the whole series, even when a changed occurrence was dragged
        start = ev.start + delta
        end = (ev.end - timedelta(days=1) if ev.all_day else ev.end) + delta
        # every date of the series moves with it; occurrences changed one by one no longer match any
        keep = kept_lines(ev, exdates=False) + ce.shifted_exdates(ev, delta)
    ...
    def delete_event(self, o):
        ...
        elif series_master(o.event) is not None:
            self.ask_scope("delete", o, {
                "this": lambda: self._series_write(o, "this", delete=True),
                "following": lambda: self._series_write(o, "following", delete=True),
                "series": lambda: self._do_delete(o)})
            return
        else:
            m.addAction(_("delete"), lambda: self._do_delete(o))
        m.exec_(QtGui.QCursor.pos())

    # ---- a series on a CalDAV server: one occurrence, from one on, or all of it -----------------

    def ask_scope(self, verb, o, actions):
        """The question every calendar asks of a repeating event. "From this one on" is left out on
        the first occurrence, where it is the whole series. Returns False when nothing was chosen."""
        master = series_master(o.event)
        first = o.event.master is None and o.start == as_datetime(master.start)
        labels = {"move": ("move only this event", "move this and the following events", "move the whole series"),
                  "edit": ("edit only this event", "edit this and the following events", "edit the whole series"),
                  "delete": ("delete only this event", "delete this and the following events", "delete the whole series")}[verb]
        m = QtWidgets.QMenu(self)
        m.addAction(_(labels[0]), actions["this"])
        if not first: m.addAction(_(labels[1]), actions["following"])
        m.addAction(_(labels[2]), actions["series"])
        return m.exec_(QtGui.QCursor.pos()) is not None

    def _series_write(self, o, scope, delta=None, delete=False, kw=None, done=None):
        """scope "this": the occurrence becomes (or stops being) an exception of its series.
        scope "following": the series ends just before it and, unless deleting, a new one starts
        there — created first, so that a failure leaves one event too many rather than a hole."""
        ev = o.event; master = series_master(ev)
        rid = ev.recurrence_id if ev.master is not None else (o.start.date() if master.all_day else o.start)
        rid_dt = as_datetime(rid)
        if kw is None and not delete:
            start = o.start + delta
            end = (o.end - timedelta(days=1) if ev.all_day else o.end) + delta
            kw = dict(summary=ev.summary, start=start.date() if ev.all_day else start, end=end.date() if ev.all_day else end, all_day=ev.all_day,
                      location=ev.location, description=ev.description, reminder=ev.reminder)
        client = self.client; cal = getattr(master, "cal_url", None)
        def work():
            if scope == "this":
                ics = ce.ics_without_occurrence(master, rid_dt) if delete else ce.ics_with_override(master, rid_dt, **kw)
                client.put(master.href, ics, etag=master.etag); return
            if not delete:
                rest = dict(kw); rest.setdefault("rrule", ce.rule_for_the_rest(master, rid_dt))
                client.create(cal, **rest)
            client.put(master.href, ce.ics_until(master, rid_dt), etag=master.etag)
        self.status.setText(_("deleting…") if delete else _("saving…"))
        after = done or (lambda _r: self.sync())
        self.run(work, after, lambda err: (self.status.setText(str(err)), self.render_current()))
    ...
    def edit_event(self, o, source=None, scope=None):
        ...
        if o is not None and scope is None and is_google(o.event) and o.event.google.get("recurring"):
            ...
            m.exec_(QtGui.QCursor.pos())
            return
        if o is not None and scope is None and not is_google(o.event) and series_master(o.event) is not None:
            master = series_master(o.event)
            self.ask_scope("edit", o, {
                "this": lambda: self.edit_event(o, o.event, "this"),
                "following": lambda: self.edit_event(o, master, "following"),
                "series": lambda: self.edit_event(o, master, "series")})
            return
        ...
        if state["occ"] is not None:
            ...
            state["start"] = o.start
            state["end"] = o.end - timedelta(days=1) if ev.all_day else o.end
            if scope == "following":
                state["rrule"] = ce.rule_for_the_rest(ev, o.start)
    ...
    def save_event(self):
        ...
        if st["href"] and st.get("gevent") is None:
            ...
            # sync may have replaced meanwhile
            src = st.get("src")
            if src:
                # the same hours and the same rule: the dates the series skips and the occurrences
                # changed one by one still fit, and are written back. Otherwise the skipped dates
                # move with the start, and the changed occurrences — matching nothing now — go.
                moved = as_datetime(start) - as_datetime(src.start)
                same = not moved and st["rrule"] == src.rrule and st["all_day"] == src.all_day
                keep = kept_lines(src, exdates=same)
                if same: overrides = [ov.lines for ov in src.overrides]
